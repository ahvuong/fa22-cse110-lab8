describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$('product-item');
    let shadow = await prodItems.getProperty('shadowRoot');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++)
    {
      ...
      let shadow = await prodItems[i].getProperty('shadowRoot');
      let button = await shadow.$('button');
      let data = await button.getProperty('innerHTML');
      data = await data.jsonValue();
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const data = await page.$$eval('product-item', () => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++)
    {
      let shadow = await prodItems[i].getProperty('shadowRoot');
      let button = await shadow.$('button');
      let data = await button.getProperty('innerText');
      data = await data.jsonValue();
      expect(data).toBe('Add to Cart');
    }
